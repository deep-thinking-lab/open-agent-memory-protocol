...
v1.3은 동일한 사용자를 위한 여러 에이전트가 동일한 백엔드에 접근할 때 백엔드가 이러한 필드로 무엇을 해야 하는지를 정의합니다. 이는 다음을 표준화합니다:
...
v1.3의 유일한 새로운 와이어 수준 추가 사항은 다음과 같습니다:

- `GET /v1/capabilities`에서 선택적 `capabilities.governance.enforcement`
- JWT 주장 또는 `OAMP-Grant`에 대한 휴대 가능한 에이전트 권한 주장 형식
- 기존 v1.2 `governance` 필드를 소비하는 규범적 백엔드 동작

v1.3은 **새로운 `KnowledgeEntry` 필드**를 도입하지 않으며 **새로운 `KnowledgeStore` 필드**도 도입하지 않습니다.
...
`governance.labels`가 없거나 비어 있을 경우, 유효한 레이블 세트는 강제를 위한 목적에서 `["behaviour"]`입니다.
...
- `mediation: "required"`는 항목에 대한 접근이 신뢰할 수 있는 중재 발급자로부터 유효한 권한을 요구함을 의미합니다.
- `mediation: "optional"`은 중재 제약이 표현되지 않음을 의미합니다.

`governance`가 존재하고 처리 값이 생략된 경우, 해당 표면에 대한 유효한 기본값은 `governed`입니다.

`mediation`이 생략된 경우, 유효한 기본값은 `optional`입니다.
...
| 주장 | 요구 사항 | 설명 |
|-------|-------------|-------------|
| `oamp_agent_id` | MUST | 호출 에이전트의 안정적인 식별자 |
| `oamp_grant_id` | MUST | 권한 인스턴스의 안정적인 식별자 |
| `oamp_read_labels` | MUST | 에이전트가 읽을 수 있는 레이블 |
| `oamp_write_labels` | MUST | 에이전트가 쓸 수 있는 레이블 |
| `oamp_sensitivity_max` | MUST | 읽기/쓰기 가능한 최고 민감도 클래스 |
| `oamp_export_full` | MAY | 전체 필터링되지 않은 내보내기가 허가되었는지 여부 |
| `iss` | 중재가 필요한 리소스에 대해 MUST; 그렇지 않으면 MAY | 권한을 발급한 기관의 안정적인 식별자 |
| `oamp_mediation_required` | MAY | 이 권한이 중재 흐름을 위해 의도되었음을 나타냅니다. |
| `oamp_task_id` | MAY | 에이전트가 수행하도록 제공된 작업 단위의 식별자 |
| `oamp_context_id` | MAY | 작업 위의 불투명한 그룹화 식별자 |

빈 `oamp_read_labels`는 읽지 않음을 의미합니다.

### 4.2 `OAMP-Grant` 헤더

JWT 베어러 토큰을 사용하지 않는 배포의 경우, 동일한 주장 객체는 `OAMP-Grant` 헤더에서 전달될 수 있습니다. 헤더 값은 주장 객체에 대한 압축 JWS여야 합니다.
...
v1.3.1 권한 하에 `oamp_task_id` 또는 `oamp_context_id`를 포함하는 쓰기가 발생할 때, 백엔드는 해당 권한에 의해 생성된 출처에 대해 `provenance.sources[*].task_id` 및 `provenance.sources[*].context_id`에 이러한 값을 찍어야 합니다. 이 필드는 설명적 귀속만을 위한 것이며 접근을 확대해서는 안 됩니다.
...
항목은 관리된 읽기를 통과하기 위해서는 다음 조건을 충족해야 합니다:
...
- 범위를 벗어난 ID에 대해 `GET /v1/knowledge/{id}`는 `404 Not Found`를 반환해야 하며, `403 Forbidden`은 반환하지 않아야 합니다.
...
- 항목의 유효한 레이블이 쓰기 권한을 벗어나는 경우, 또는
...
| 필드 | 유형 | 요구 사항 | 설명 |
|-------|------|-------------|-------------|
| `enforcement.supported` | boolean | `enforcement`가 존재할 경우 MUST | 백엔드가 v1.3 강제 규칙을 적용합니다. |
| `enforcement.spec_version` | string | MUST | 구현된 v1.3 사양 라인 |
| `enforcement.label_hierarchy` | string | MUST | 이 초안에 대한 `dotted-prefix` |
| `enforcement.reserved_top_level_labels` | string 배열 | MUST | 예약된 상호 운용 가능한 최상위 레이블 |
| `enforcement.grant_transport` | string 배열 | MUST | 지원되는 권한 전송 메커니즘 |
...
| `enforcement.mediation` | object | MAY | 중재 지원 및 신뢰할 수 있는 발급자 식별자 |
| `enforcement.provenance_query` | string 배열 | MAY | 지원되는 provenance-context 필터 (`task_id`, `context_id`) |
...
`scope_denied_read`는 보호된 항목 내용을 기록해서는 안 되며, 에이전트 표면에서 필터링된 항목 ID를 기록하지 않도록 해야 합니다.
...
- `1.3.0` 버전 문자열만으로 휴대 가능한 withheld-result 의미를 유추해서는 안 됩니다.

### 8.3 권한 없는 토큰

v1.3을 에이전트 표면에 대해 강제하는 백엔드에서, 사용 가능한 `oamp_read_labels`가 없는 토큰은 읽지 않음으로 처리해야 합니다.
